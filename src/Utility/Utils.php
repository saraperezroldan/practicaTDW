...
class Utils
{
     ...
     * defined in .env.local file + (.env.local.docker || .env.local.local)
     *
    ...
    public static function loadEnv(string $dir): void
    {
        ...
        try {
            ...
            if (file_exists($dir . '/.env.local')) {
                ...
                fwrite(STDERR, 'ERROR: no existe el fichero .env.local' . PHP_EOL);
                exit(1);
            }

            // Overload (if they exist) with .env.local.docker or .env.local.local
            if (isset($_SERVER['DOCKER']) && file_exists($dir . '/.env.local.docker')) {
                $dotenv = Dotenv::createMutable($dir, '.env.local.docker');
                $dotenv->load();
            } elseif (file_exists($dir . '/.env.local.local')) {
                $dotenv = Dotenv::createMutable($dir, '.env.local.local');
                $dotenv->load();
            }
        } catch (Throwable $e) {
            fwrite(
